...
export function getCategorizedMembers({
	...
	const currentAssignments = assignments.filter(a => a.weekday === selectedDay && a.slot_type === selectedSlot);
	const currentSpecificAssignments = specificAssignments.filter(a => a.weekday === selectedDay && a.slot_type === selectedSlot);
	...
	// Categorize members
	const availableMembers = members.filter(m => !assignedMemberIds.has(m.id) && !absentMemberIds.has(m.id));
	const assignedMembersData = members.filter(m => assignedMemberIds.has(m.id));
